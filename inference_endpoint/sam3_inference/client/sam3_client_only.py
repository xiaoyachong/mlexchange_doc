"""
SAM3 Client - API-only implementation
This replaces your existing utils/sam3_utils.py

Place this file at: utils/sam3_utils.py
"""
import logging
import os
import requests
...
SAM3_INFERENCE_URL = os.getenv(
    "SAM3_INFERENCE_URL", 
    "http://sam3-inference:5001/invocations"
)
            ...
            try:
                ...
                if "predictions" in result:
                    ...
                    if not prediction.get("success", False):
                        logger.error(f"API error: {prediction.get('error')}")
                        return None
                    
                    # Decode masks
                    masks = [
                        torch.from_numpy(self._decode_mask(enc)) 
                        for enc in prediction["masks"]
                    ]
